...
## 이미지를 저장할 폴더 경로
folder_path = 'C:\\Users\\User\\Desktop\\project4\\my_face_exam2'
## 이미지 저장 폴더가 없다면 폴더 생성
if not os.path.exists(folder_path):
    os.makedirs(folder_path)
    print('my_face2 폴더를 생성합니다.')
...
def capture_owner_images(num_images=1000):  # num_images에 숫자를 입력한만큼 이미지 저장
    ...
    while count < num_images:
        ...
        for (x, y, w, h) in faces:
            ...
            ## 파일 이름 설정 및 저장 (폴더 경로와 파일 이름을 포함)
            face_file = os.path.join(folder_path, f"face_{count}.jpg")
